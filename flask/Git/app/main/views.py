...
from app.models import Engpass, User, Drug, Producer, Contact, Essentiell
...
def index():
    ...
    essentielle = Essentiell.objects()
    if current_user.is_authenticated:
        current_user.update_last_seen()
    return render_template('main/index.html', engpaesse=engpaesse, essentielle = essentielle)
...
@main.route('/remove_engpass/<int:enr>', methods=['GET', 'POST'])
@login_required
def remove_engpass(enr):
    engpass = Engpass.get_by_enr(enr)
    engpass.delete()
    return redirect(url_for('main.index'))