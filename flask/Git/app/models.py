...
class Essentiell(db.Document):
    """docstring for Essentiell"""
    # int
    enr = db.IntField()

    @staticmethod
    def get_by_enr(enr):
        return Essentiell.objects.get(enr=enr)

    def __repr__(self):
        return '<Essentiell {}>'.format(self.enr)
...
class Engpass(db.Document):
    ...
    # bool
    alternative = db.BooleanField(default=False)
    inform_expert_group = db.BooleanField(default=False)
